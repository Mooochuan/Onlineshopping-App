...
class Product extends Component {
  ...
  render() {
    ...
    const unitAmount = price?.unit_amount ?? 0; // e.g. 999 for 9.99
    ...
    return (
        <div key={id} className="product row">
          ...
          <div className="product-details col-xs-12 col-sm-6 col-lg-4">
            ...
            {/* Single price only, no size/inventory selection anymore */}
            <PayButton
                amount={unitAmount}
                price={price} // in case we want to pass the price ID
                name={name}
                caption={caption}
            />
          </div>
